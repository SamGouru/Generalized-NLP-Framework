...
from query_decider import query_decider, pop_decider, cap_decider, size_decider, howmany_decider
...
MiniTester = ""
TESTER2 = ""
SelectDecider = query_decider(sentence)
print ("SelectDecider: %s" % SelectDecider)

PopDecider = pop_decider(sentence)
print ("PopDecider: %s" % PopDecider)

SizeDecider = size_decider(sentence)
print ("SizeDecider: %s" % SizeDecider)

HowDecider = howmany_decider(sentence)
print ("HowDecider: %s" % HowDecider)


CapDecider = ""
DistDecider = ""

if ("capital" in sentence or "Capital" in sentence):
	CapDecider = "true"

if ("district" in sentence or "District" in sentence):
	DistDecider = "true"
...
for idx, val in enumerate(SELECT_L):
    ...
    if (len(SELECT) == 0 and SelectDecider == "peddha"):
	
        if (PopDecider == "city"):
            SELECT += 'Select ' + " MAX(" + val[1] + ")"
            break

        elif (PopDecider == "country"):
            SELECT += 'Select ' + " MAX(" + val[1] + ")"
            break

        elif (SizeDecider == "countrysize" or SizeDecider == "citysize"):
            SELECT += 'Select ' + " MAX(" + val[1] + ")"
            break
			
    elif (len(SELECT) == 0 and SelectDecider == "chinna"):
	
        if (PopDecider == "city"):
            SELECT += 'Select ' + " MIN(" + val[1] + ")"
            break
    ...
    elif (len(SELECT) == 0 and SelectDecider == "avg"):
	
        if (PopDecider == "city"):
            SELECT += 'Select ' + " AVG(" + val[1] + ")"
            break
    ...
    elif (len(SELECT) == 0 and SelectDecider == "sum"):
	
        if (PopDecider == "city"):
            SELECT += 'Select ' + " SUM(" + val[1] + ")"
            break
    ...
    elif (len(SELECT) == 0 and SelectDecider == "count"):
	
        SELECT += 'Select ' + " COUNT(" + val[1] + ")"
        break
    ...
    elif (len(SELECT) == 0 and SizeDecider == "citysize"):
        SELECT += 'Select ' + 'city.' + val[1]
        break
	
    elif (len(SELECT) == 0 and SizeDecider == "countrysize"):
        SELECT += 'Select ' + 'country.' + val[1]
        break

    elif (len(SELECT) == 0 and SelectDecider == ""):

        if (PopDecider == "city"):
            SELECT += 'Select ' + "city.Name , city" + "." + val[1]
            ...
            SELECT += 'Select ' + "country.Name , country" + "." + val[1]
            ...
            if (CapDecider == "true"):
                ...
                if (len(SELECT) == 0):
                    print ("CapDecider: %s" % CapDecider)
                    SELECT += 'Select city.Name, ' + val[0] + "." + val[1]
                    ...
                    print ("CapDecider: %s" % CapDecider)
                    SELECT += ' , city.Name, ' + val[0] + "." + val[1]
    
            if (DistDecider == "true"):
			
                if (len(SELECT) == 0):
                    SELECT += 'Select city.District, ' + val[0] + "." + val[1]
                    ...
                    SELECT += ' , city.District, ' + val[0] + "." + val[1]
...
for idx, val in enumerate(FROM_L):
    ...
    if (SelectDecider == "peddha" and PopDecider == "" or SelectDecider == "chinna" and PopDecider == ""):
	
        FROM += ' FROM ' + val[0]

        FROM_Added.append(val[0])

        break
		
    elif (PopDecider == "city" and SelectDecider == "peddha" or PopDecider == "city" and SelectDecider == "chinna" 
	or SizeDecider == "citysize"):
    ...
    elif (PopDecider == "country" and SelectDecider == "peddha" or PopDecider == "country" and SelectDecider == "chinna" 
	):
    ...
    elif (CapDecider == "true" and SizeDecider == "countrysize" ):
        
        FROM += ' FROM ' + val[0] + ' INNER JOIN ' + 'country' + ' ON ' + TESTER1 + " = " + TESTER2

        FROM_Added.append(val[0])
		
        break

    elif (len(FROM) == 0):                    # if empty
        ...
        if (val[0] not in FROM_Added):      # test if we've already added this table before
            ...
            if ("language" in sentence and "CountryCode" in sentence):
                FROM += ' INNER JOIN ' + val[0] + ' ON ' + 'city.CountryCode' + " = " + 'countrylanguage.CountryCode'
                FROM_Added.append(val[0])
			
            else:
                FROM += ' INNER JOIN ' + val[0] + ' ON ' + TESTER1 + " = " + TESTER2
...
#query = "SELECT city.Name, city.Population FROM city WHERE city.Name = 'Paris'"
